/**
 * zipToState — lightweight US ZIP code prefix → state abbreviation lookup.
 * Covers all 50 states + DC using the first 3 digits of the ZIP.
 * Accuracy: ~95%+ for standard 5-digit ZIPs. Edge cases (multi-state prefixes)
 * default to the most populous state for that prefix.
 */
...
/**
 * Returns the 2-letter US state abbreviation for a given ZIP code.
 * Falls back to "TX" if the ZIP is unrecognized.
 */
export function zipToState(zip: string): string {
  if (!zip || zip.length < 3) return "TX";
  const prefix = zip.substring(0, 3).padStart(3, "0");
  return ZIP_PREFIX_MAP[prefix] ?? "TX";
}
