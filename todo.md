...
## Full Render Migration (replacing all Manus infrastructure)

- [ ] Create Render PostgreSQL database
- [ ] Switch Drizzle ORM driver from mysql2 to postgres.js (pg)
- [ ] Update drizzle/schema.ts for PostgreSQL compatibility (types, serial, etc.)
- [ ] Update drizzle.config.ts to use PostgreSQL dialect
- [ ] Run pnpm db:push against Render PostgreSQL to create schema
- [ ] Migrate existing data from Manus MySQL to Render PostgreSQL
- [ ] Update DATABASE_URL env var on Render web service
- [ ] Replace Manus OAuth with self-contained JWT auth (no external OAuth dependency)
- [ ] Replace Manus storage proxy with direct S3/R2 or remove if unused
- [ ] Replace Manus notifyOwner helper with email (Resend/SendGrid) or remove
- [ ] Replace Manus LLM helper references if any are used
- [ ] Push updated code to GitHub and trigger Render redeploy
- [ ] Verify end-to-end: lead form, database writes, admin panel, all pages
